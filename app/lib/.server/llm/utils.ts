...
const PROVIDER_REGEX = /^[\s\S]*?(@provider:(\w+))/i;
...
export function extractPropertiesFromMessage(message: Omit<Message, 'id'>): {
   ...
   */
  const model = modelMatch ? modelMatch[2] : DEFAULT_MODEL;

  /*
   * Extract provider
   * const providerMatch = message.content.match(PROVIDER_REGEX);
   */
  const provider = providerMatch ? providerMatch[2] : DEFAULT_PROVIDER.name;
