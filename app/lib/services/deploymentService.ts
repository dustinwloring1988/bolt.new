  ...
  static async deployToVercel(token: string, projectName?: string): Promise<DeploymentResult> {
    try {
      ...
      if (!response.ok) {
        ...
        if (error instanceof Error) {
          throw new Error(
            (error as any).error?.message || error.message || `Vercel deployment failed: ${response.statusText}`,
          );
        } else {
        ...
        case 'netlify':
          ...
          if (typeof netlifyData === 'object' && netlifyData !== null && 'state' in netlifyData) {
            return {
              ...
              url:
                (netlifyData as any).state === 'ready'
                  ? (netlifyData as any).ssl_url || (netlifyData as any).deploy_ssl_url
                  : undefined,
              error:
                (netlifyData as any).state === 'error' || (netlifyData as any).state === 'failed'
                  ? (netlifyData as any).error_message
                  : undefined,
            };
          } else {
            throw new Error('Invalid response from Netlify');
